...
# get all dates within 2 days of sampling
...
MLTP_dates_all <- c(MLTP_dates, (MLTP_dates + 1), (MLTP_dates + 2), (MLTP_dates - 1), (MLTP_dates - 2))
...
for(day in LTP_dates_all){
  ...
  if(nrow(data) > 1) {
    filename = paste0('downloaded_csvs/', as.character(as.Date(day)), '.csv')
    write_csv(x = data, filename)
  }else{print("No data")}
}

# Relevant tracks = 242 + 1210
